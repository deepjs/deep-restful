			...
			if(schema)
			{
				...
				if(schema.ownerRestiction)
				{
					...
					if(!content[schema.ownerRestiction])
						content[schema.ownerRestiction] = deep.Promise.context.session.user.id;
					else if(content[schema.ownerRestiction] !== deep.Promise.context.session.user.id)
						return deep.errors.Owner();
				}
		...
		"dq.up::./get":deep.compose.around(function(old){
			return function(parsed, options){
				...
				if (schema && schema.ownerRestriction)
					if (deep.Promise.context.session && deep.Promise.context.session.user)
						q += "&" + schema.ownerRestriction + "=" + deep.Promise.context.session.user.id;
					else
			...
			return function(parsed, options)
			{
				...
				if(schema)
				{
					...
					if(schema.ownerRestriction)
					{
						if(!deep.Promise.context.session || !deep.Promise.context.session.user)
							return deep.errors.Owner();
						filter += "&"+schema.ownerRestriction+"="+deep.Promise.context.session.user.id;
					}
