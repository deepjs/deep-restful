...
define(["require", "deepjs/deep", "./store", "deepjs/lib/nodes"], function(require, deep, Store, nodes) {
	/**
	...
	var filterPrivate = Store.filterPrivate = function(method) {
		return function(old) {
			return function(object, options) {
				...
				deep.Querier.remove(object, ".//!?_schema.private=true", schema);
				return deep.when(old.call(this, object, options))
					.done(function(result) {
						...
						deep.Querier.remove(result, ".//!?_schema.private=true", schema);
						return result;
					});
			};
		};
	...
	var applyPut = function(item, query, object) {
		if (query) {
			deep.Querier.replace(item, query, object);
			return item;
		}
		return object;
	};
	...
	var applyPatch = function(item, query, object) {
		if (query) {
			var r = deep.query(item, query, {
				fullOutput: true,
				allowStraightQueries: false
			});
				...
				return deep.when(old.call(this, id, options))
					.done(function(res) {
						...
						if (!id || id[0] == '?' || id[0] == "*") {
							if (self._deep_collection_) {
								if (!options.noPrivates)
									deep.Querier.remove(res, ".//!?_schema.private=true", {
										type: "array",
										items: schema
									});
								return res;
						...
						if (schema && !options.noPrivates)
							deep.Querier.remove(res, ".//!?_schema.private=true", schema);
						if (parsed.path)
							res = deep.utils.fromPath(res, parsed.path, "/");
						return res;
					});
		...
		"dq.bottom::./!": {
			...
			checkReadOnly: function(object, data, options, schema) {
				...
				if (this._deep_object_)
					schema = deep.utils.schemaByValuePath(schema, options.id + (options.path || ""), '/');
				var nodes = deep.query(data, ".//?_schema.readOnly=true", {
					fullOutput: true,
					schema: schema
				});
