...
define(["require", "deepjs/deep", "./range", "./store", "./store-sheet"], function(require, deep, ranger) {
	 ...
	 */
	deep.Collection = deep.Classes(deep.Store,
		function(protocol, collection, schema, options) {
			...
			put: function(object, options) {
				...
				if (col._deep_ocm_)
					col = col();
				deep.Querier.replace(col, "./*?id=" + id, object);
				return object;
			},
			/**
			 * @method patch
			 ...
			 * @return {deep.NodesChain} a chain that hold the patched object and has injected values as success object.
			 */
			...
			del: function(id, options) {
				...
				if (options.filter)
					q += options.filter;
				var removed = deep.Querier.remove(col, q);
	...
	deep.coreUnits.push("req::deep-restful/tests/collections");
